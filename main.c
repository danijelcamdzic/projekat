...
/* ------------------------------------------ Calculating the ERROR  ---------------------------------------- */
	...
	*AccelZError = *AccelZError/100.0;	/**< Don't want to count gravity as noise! */	
	
	float AccelAngleX = atan(*AccelYError / sqrt(pow(*AccelXError, 2) + pow(*AccelZError, 2)))*180/PI;				/**< Roll y, z*/
	float AccelAngleY = atan(-1*(*AccelXError) / sqrt(pow(*AccelYError, 2) + pow(*AccelZError, 2)))*180/PI;			/**< Pitch x, z */
	...
	*AccelXError = 0.0;
	*AccelYError = 0.0;
	*AccelZError = 0.0;	/**< Don't want to count gravity as noise! */	
...
void CalibrateMagnetometer(void) {
	...
	for(i = 0; i < calibration_count; i++) {				/**< Do 60000 measurements */	
    ReadMagnetometer(); 
...
void SendToUART(uint8_t *pbuf) {
		...
		for (uint32_t i=0; i<size; i++){
			...
			while(!(USART2->ISR & USART_ISR_TC));			/**< Check transmission complete flag in order to continue */	
		}
}
